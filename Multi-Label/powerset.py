...
allClassifiers = ["XGBoost","RandomForest","SVM","LogisticRegression"]
def chooseClassifier(classifier,X_train,y_train):
    if(classifier == "XGBoost"):
        model = LabelPowerset(XGBClassifier(random_state = 0)).fit(X_train,y_train)
    elif(classifier == "RandomForest"):
        model = LabelPowerset(RandomForestClassifier(n_estimators = 1000, criterion = 'entropy', random_state = 0)).fit(X_train,y_train)
    elif(classifier == "SVM"):
        model = LabelPowerset(LinearSVC(random_state=0)).fit(X_train,y_train)
    elif(classifier == "LogisticRegression"):
        model = LabelPowerset(LogisticRegression(random_state=0)).fit(X_train,y_train)
    return model
...
for c in allClassifiers:
    for train_index, test_index in kf.split(X):
        ...
        hamming, subset_accuracy, recall, precision, f1, coverage, aps, rankingloss = powerset(X_train, X_test, y_train, y_test,c)
    ...
    plt.title('Metrics of '+c)
    plt.savefig("POWERSET/"+c+".png", transparent=True,dpi=200)
